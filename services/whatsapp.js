  ...
  if (msg.body === "!ping reply") {
    ...
    if (result.intent === "Default Welcome Intent") {
      ...
      if (result.intent === "today") {
        const getRoznamcha = await pdf.createRoznamchaPDF(result.response);
      ...
      else if (result.intent === "yesterday") {
        const getRoznamcha = await pdf.createRoznamchaPDF(result.response);
        if (getRoznamcha.data) {
          const media = MessageMedia.fromFilePath(
            "./pdf/roznamcha_" + new moment().format("DD-MMM-YYYY") + ".pdf"
